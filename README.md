...
ぞろ屋式ホームページ作成工程のうち、ライセンス認証を必要としない設計・本文・ファーストビュー選定まで完成しています。採用FVは`fv-23` Stagger Reveal／A「一行ずつ出る」です。現在はGitHub Pagesの入口`index.html`と、通信費の料金差を計算できる`mobile.html`、保険料を同条件で比較する`insurance.html`を、画像を使わない暫定公開プレビューとして配置しています。30点以上の画像を使う正式版は、正規ライセンス認証後に進めます。
